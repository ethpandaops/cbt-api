...
func TestQueryParameterValidation(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			path:                  "/api/v1/fct_block",
			method:                http.MethodGet,
			queryParams:           "slot_eq=xasdfsdfafds",
			expectedStatus:        http.StatusBadRequest,
			expectedCode:          codes.InvalidArgument,
			expectedMessagePrefix: "parameter 'slot_eq' must be a valid unsigned 32-bit integer",
		},
		{
			name:                  "invalid uint32 parameter value (negative)",
			path:                  "/api/v1/fct_block",
			method:                http.MethodGet,
			queryParams:           "slot_eq=-123",
			expectedStatus:        http.StatusBadRequest,
			expectedCode:          codes.InvalidArgument,
			expectedMessagePrefix: "parameter 'slot_eq' must be a valid unsigned 32-bit integer",
		},
		{
			...
			path:                  "/api/v1/fct_block",
			method:                http.MethodGet,
			queryParams:           "slot_eq=99999999999999999999",
			expectedStatus:        http.StatusBadRequest,
			expectedCode:          codes.InvalidArgument,
			expectedMessagePrefix: "parameter 'slot_eq' must be a valid unsigned 32-bit integer",
		},
		...
		},
		{
			...
			path:           "/api/v1/fct_block",
			method:         http.MethodGet,
			queryParams:    "slot_gte=100&slot_lte=200",
			expectedStatus: http.StatusOK,
		},
	}
